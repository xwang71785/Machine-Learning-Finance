# -*- coding: utf-8 -*-
"""
Created on Thu Apr 13 11:05:35 2017

Beat Pong with Policy Gradients
"""

""" Trains an agent with (stochastic) Policy Gradients on Pong. Uses OpenAI Gym. """ 
#matrix math 
import numpy as np  
#for loading and saving models. Pickling is the process whereby a Python object 
...
#1 We dont have the correct labels so as a fake label we substitute the action 
#we happened to sample from the policy  
...
#3 runs on a continuously changing dataset (the episodes), scaled by the advantage, 
#and we only want to do one (or very few)  
#updates based on each sampled dataset. 
#4 can be used to backprop when nodes are stochastic (!!, more on this later) 
 
#Step 4 - PG vs Humans 
#Policy Gradients have to actually experience a positive reward, and experience 
#it very often in order to eventually and slowly shift the policy parameters towards  
#repeating moves that give high rewards. With our abstract model, humans can figure 
#out what is likely to give rewards without ever actually experiencing the  
#rewarding or unrewarding transition. I dont have to actually experience crashing 
#my car into a wall a few hundred times before I slowly start avoiding to do so. 
...
if resume: 
    ...
    #Imagine that your weights are initially very close to 0. What happens is 
    #that the signals shrink as it goes through each  
    #layer until it becomes too tiny to be useful. Now if your weights are too big, 
    #the signals grow at each layer  
    #it passes through until it is too massive to be useful. 
    #By using Xavier initialization, we make sure that the weights are not too 
    #small but not too big to propagate accurately the signals. 
...
def policy_forward(x): 
    ...
    #will be able to detect various game scenarios 
    #(e.g. the ball is in the top, and our paddle is in the middle) 
    h = np.dot(model['W1'], x) 
...
def policy_backward(eph, epdlogp): 
    ...
    #compute updated derivative with respect to weight 2. It's the parameter 
    #hidden states transpose * gradient w/ advantage (then flatten with ravel()) 
...
while True: 
    ...
    #if it was apart of the model, we'd have to use a reparametrization trick 
    #(a la variational autoencoders. so badass) 
    ...
    #to be taken (see http://cs231n.github.io/neural-networks-2/#losses if confused) 
    ...
    if done: # an episode finished 
     ...
     #the strength with which we encourage a sampled action is the weighted sum 
     #of all rewards afterwards, but later rewards are exponentially less important 
